...
class DhgateOrderDisputecloseListResultInfo
{
    ...
    /*
     * disputeCloseDate 	String 	必须 	纠纷关闭时间 	日期格式：yyyy-MM-dd HH:mm:ss,精确到秒；示例值：2014-01-12 18:20:21
     */
    private $disputeCloseDate;
    public function GetDisputeCloseDate(){
        return $this->disputeCloseDate ;
    }
    /*
     * disputeStatus 	String 	必须 	纠纷处理状态 	109001：售后纠纷开启,109002：售后纠纷裁决中,空值表示没有售后纠纷，107001:平台纠纷开启,107002: 平台纠纷-dhgate审核中，107003：平台纠纷-裁决方案执行中，106001：协议纠纷-等待协议达成，106003：协议纠纷-协议已达成
     */
    ...
    * isWarn 	Boolean 	必须 	是否需要特别注意的订单（如高风险订单、售后纠纷订单等） 	值为false或null表示订单正常，true表示此订单需要特别注意一下，例如，防止高风险订单直接发货
    ...
    public function setStdResult($stdResult) {
        ...
        if (array_key_exists ( "disputeCloseDate", $this->stdResult )) {
            $this->disputeCloseDate = $this->stdResult->{"disputeCloseDate"};
        }
        if (array_key_exists ( "disputeStatus", $this->stdResult )) {
            $this->disputeStatus = $this->stdResult->{"disputeStatus"};
        }
    ...
    public function setArrayResult($arrayResult) {
        ...
        if (array_key_exists ( "disputeCloseDate", $this->arrayResult )) {
            $this->disputeCloseDate = $arrayResult['disputeCloseDate'];
        }
        if (array_key_exists ( "disputeStatus", $this->arrayResult )) {
            $this->disputeStatus = $arrayResult['disputeStatus'];
        }
