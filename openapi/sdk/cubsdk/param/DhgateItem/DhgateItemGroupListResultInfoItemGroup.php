...
use ArrayObject;
class DhgateItemGroupListResultInfoItemGroup{
...
/*
 * itemChildGroupList 	ItemChildGroup[] 	必须 	产品属性值列表 	产品组子组列表，若无子组，则该列表返回空
 */
    private $itemChildGroupList;
    public function GetItemChildGroupList(){
        return $this->itemChildGroupList;
    }
/*
 ...
 */
    ...
    public function setStdResult($stdResult) {
        ...
        if (array_key_exists ( "itemChildGroupList", $this->stdResult )) {
            $this->itemChildGroupList = $this->stdResult->{"itemChildGroupList"};
        }
        if (array_key_exists ( "itemChildGroupList", $this->stdResult )) {
            $itemChildGroupListResult=$this->stdResult->{"itemChildGroupList"};
            $object = json_decode ( json_encode ( $itemChildGroupListResult ), true );
            $this->itemChildGroupList = array ();
            for($i = 0; $i < count ( $object ); $i ++) {
                $arrayobject = new ArrayObject ( $object [$i] );
                $DhgateItemGroupListResultInfoItemGroupItemChildGroup=new DhgateItemGroupListResultInfoItemGroupItemChildGroup();
                $DhgateItemGroupListResultInfoItemGroupItemChildGroup->setArrayResult($arrayobject );
                $this->itemChildGroupList[$i] = $DhgateItemGroupListResultInfoItemGroupItemChildGroup;
            }
        }
    ...
    public function setArrayResult($arrayResult) {
        ...
        if (array_key_exists ( "itemChildGroupList", $this->arrayResult )) {
            $this->itemChildGroupList = $arrayResult['itemChildGroupList'];
        }
