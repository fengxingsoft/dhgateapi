...
class DhgateItemAddParam
{
    /*
     * afterSaleTemplateId 	String 	可选 	产品售后模板id 	3C类产品上传必填;通过获取售后模板接口（dh.item.aftersale.list）获取；示例值：82558025181257728；
     */
    ...
    /*
     * itemAttrGroupList 	ItemAttrGroup[] 	必须 	兼容属性列表 	兼容属性列表信息
     */
    ...
    /*
     * itemAttrList 	ItemAttr[] 	必须 	产品属性列表 	产品属性列表信息
     */
    ...
    /*
     * itemBase 	ItemBase 	必须 	产品基础信息 	产品基础信息，包含短描，长描，产品名称等信息
     */
    public function setItemBase($itemBase)
    {
    ...
    /*
     * itemImgList 	ItemImg[] 	必须 	产品图片列表 	产品图片列表信息在,注意新旧图片地址路径不同，且上传产品时必须设置一张图片为站外推广图片(type=3)
     */
    ...
    /*
     * itemInventory 	ItemInventory 	可选 	产品备货信息，无备货时可不传 	产品备货信息，无备货时可不传
     */
    ...
    /*
     * itemSaleSetting 	ItemSaleSetting 	必须 	产品销售属性设置 	产品销售属性信息
     */
    ...
    /*
     * itemSkuList 	ItemSku[] 	必须 	产品SKU列表 	产品SKU列表信息，Sku列表数量=产品属性中规格属性数量*自定义规格属性数量
     */
    ...
    /*
     * itemSpecSelfDefList 	ItemSpecSelfDef[] 	必须 	自定义规格列表 	一个产品最多二十个自定义规格
     */
    ...
    /*
     * itemWholesaleRangeList 	ItemWholesaleRange[] 	必须 	产品折扣区间 	折扣区间百分比，即折扣值，如卖家设置产品折扣率为95折，则该入参为0.05；包含以下参数:discount=折扣率, startQty=起批数量
     */
    ...
    /*
     * sizeTemplateId 	String 	可选 	尺码模板id 	可通过卖家获取尺码模板列表接口（dh.item.template.list）；
     */
